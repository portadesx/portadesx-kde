...
export DISPLAY=:8
dbus-launch --exit-with-session startplasma-x11' >> /home/portadesx/.vnc/xstartup

# Create script for starting VNC server
echo '#!/bin/sh
export USER=portadesx
export HOME=/home/portadesx
vncserver -name remote-desktop -localhost no :8
echo 'VNC server address: 127.0.0.1:8 Password: 1234567890'' >> /usr/local/bin/startvnc
...
export HOME=/home/portadesx
vncserver -kill :8
rm -rf /home/portadesx/.vnc/localhost:8.pid
rm -rf /tmp/.X8-lock
rm -rf /tmp/.X11-unix/X8' >> /usr/local/bin/stopvnc
